...
(async function() {
    ...
    // Función para inyectar script externo
    function injectExternalScript(url, attributes = {}) {
    ...
    async function injectChameleonLoader() {
        ...
        try {
            console.log('[Chameleon Injector] Getting session seed...');
            ...
            console.log('[Chameleon Injector] Injecting bootstrap script...');
            
            // Inyectar primero el bootstrap con los datos embebidos
            const bootstrapUrl = chrome.runtime.getURL('content/bootstrap-init.js');
            injectExternalScript(bootstrapUrl, {
                'data-init': JSON.stringify(initData)
            });
            
            // Esperar un momento para que el bootstrap se ejecute
            await new Promise(resolve => setTimeout(resolve, 50));
            
            console.log('[Chameleon Injector] Injecting loader script...');
            
            // Ahora inyectar el loader
            const loaderUrl = chrome.runtime.getURL('content/chameleon-loader.js');
            const loaderScript = injectExternalScript(loaderUrl);
            ...
            loaderScript.onerror = (error) => {
                console.error('[Chameleon Injector] Failed to inject loader:', error);
    ...
    function waitForDOM() {
        return new Promise((resolve) => {
            if (document.head) {
                ...
                // Timeout de seguridad - Fix: Use the observer variable correctly
                setTimeout(() => {
                    observer.disconnect();
                    resolve();
                }, 5000);
