...
(function() {
    ...
    class MetaProxyInterceptor {
        ...
        interceptErrorStackTraces() {
            ...
            // Fix: Prevent cyclic prototype issues
            try {
                // Only set prototype if it's different
                if (Object.getPrototypeOf(window.Error) !== OriginalError) {
    ...
    // Fix: Prevent cyclic prototypes for PluginArray
    try {
        if (typeof PluginArray !== 'undefined' && PluginArray.prototype) {
            // Store original prototype
            const originalPluginArrayProto = PluginArray.prototype;
            
            // Create a safe fake prototype chain
            window.__chameleonOriginalPluginArray = originalPluginArrayProto;
        }
    } catch (e) {
        console.warn('[Chameleon] Could not store PluginArray prototype:', e);
    }
